...
public class SearchLocationServiceImpl implements SearchLocationService {
	...
	@Override
	/**
	 * Service method which calls the Foursquare API for which we need to pass the
	 * 'client_id','client_secret','api version',''intent' and 'near' on which it
	 * returns us the information of the location passed in 'near' .
	 * 
	 * @param query
	 * @param category
	 * @return List<Foursquare> results else null;
	 */
	public List<Foursquare> getLocationDetailsByCategory(String query, String category) {
		try {
			String data = "client_id=" + env.getProperty("foursquare.client_id").trim() + "&client_secret="
					+ env.getProperty("foursquare.client_secret").trim() + "&v="
					+ env.getProperty("foursquare.verion").trim() + "&near=" + query + "&intent=browse";
			...
			logger.debug("Call to getLocationDetails");
	...
	/**
	 * Method converts the BufferReader result into JSON and parse the JSON to the
	 * information of th passed category.
	 * 
	 * @param rd
	 * @param category
	 * @return List<Foursquare> results else null;
					...
					if (responseObject.getJSONObject("response").has("venues")) {
						...
						for (int iCount = 0; iCount < venuesArray.length(); iCount++) {
							...
							if (venuesArray.getJSONObject(iCount).getJSONArray("categories").length() > 0) {
								if (venuesArray.getJSONObject(iCount).getJSONArray("categories").getJSONObject(0)
										.has("icon")) {
									categoryFromAPI = venuesArray.getJSONObject(iCount).getJSONArray("categories")
											.getJSONObject(0).getString("name").trim();
									if (category != null)
										if (categoryFromAPI.equalsIgnoreCase(category.trim()))
											fs.setCategory(venuesArray.getJSONObject(iCount).getJSONArray("categories")
							...
							fs.setName(venuesArray.getJSONObject(iCount).has("name")
									? venuesArray.getJSONObject(iCount).getString("name")
									: "");
							...
							fs.setAddress(venuesArray.getJSONObject(iCount).getJSONObject("location").has("address")
									...
									: "");
							fs.setPostalCode(
									venuesArray.getJSONObject(iCount).getJSONObject("location").has("postalCode")
											? venuesArray.getJSONObject(iCount).getJSONObject("location")
							...
							JSONArray formattedAdd = venuesArray.getJSONObject(iCount).getJSONObject("location")
							...
							fs.setFormattedAddress(
									formattedAddStr.substring(0, formattedAddStr.lastIndexOf(",")) + ".");
